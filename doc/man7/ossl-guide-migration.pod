...
=head1 OPENSSL 4.0
=head3 Deprecation of library-specific error reason codes

Library-specific error reason codes (such as B<SSL_R_*>, B<EVP_R_*>, etc.)
are deprecated. New code should use the machine-readable B<ERR_R_*> error
codes instead, with human-readable detail provided via the format string
argument to L<ERR_raise_data(3)> or "the SSLfatal_data() macro".
...
See L<ERR_R_XXX(7)> for the complete list of machine-readable error codes.
