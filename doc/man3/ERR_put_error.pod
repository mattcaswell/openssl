...
with any sub-library code. These are the stable, documented error codes that
applications can rely on for programmatic error handling. Human-readable
detail should be provided via the B<fmt> argument to ERR_raise_data() rather
than encoded in the reason code itself.

For example, instead of:

 ERR_raise(ERR_LIB_SSL, SSL_R_BAD_PSK_IDENTITY);

use the equivalent machine-readable code with a descriptive format string:

 ERR_raise_data(ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT,
                "PSK identity is empty, violates RFC 8446");

The complete list of machine-readable error codes can be found in
L<ERR_R_XXX(3)>.

=back

The library-specific B<*_R_*> reason codes (such as B<SSL_R_*>, B<EVP_R_*>,
etc.) are deprecated.  They are retained for backward compatibility but new
code should use the B<ERR_R_XXX> codes instead.  The library-specific codes
may change between releases as OpenSSL's internals evolve.
