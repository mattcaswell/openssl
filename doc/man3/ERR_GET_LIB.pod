...
L<ERR_R_XXX(3)>) for control flow decisions and programmatic error handling.
...
etc.) are deprecated. They may change at any time (even in patch releases of
OpenSSL) and should not be used for control flow decisions. A particular
library-specific code can only be considered meaningful for control flow
decisions if it is explicitly documented as such.
