...
err:
    /* No valid transition found */
    SSLfatal_data(s, SSL3_AD_UNEXPECTED_MESSAGE, ERR_R_WRONG_STATE, "wrong state in client read transition");
    return 0;
}
...
static WRITE_TRAN ossl_statem_client13_write_transition(SSL_CONNECTION *s)
{
    ...
    default:
        /* Shouldn't happen */
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in do_compressed_cert");
    ...
    confunc_f *confunc, int *mt)
{
    ...
    default:
        /* Shouldn't happen */
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_WRONG_STATE, "wrong state in client construct message");
        return 0;
    ...
    PACKET *pkt)
{
    ...
    default:
        /* Shouldn't happen */
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in ossl_statem_client_construct_message");
    ...
    WPACKET *pkt)
{
    ...
    if (s->ext.ech.es->entries != NULL) {
        if (ossl_ech_pick_matching_cfg(s, &ee, &suite) != 1 || ee == NULL) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_UNSUPPORTED, "unsupported in client hello aux");
            return 0;
        }
        ...
        if (s->ext.ech.outer_hostname == NULL && ee->public_name != NULL) {
            ...
            if (s->ext.ech.outer_hostname == NULL) {
                SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_client_hello_aux");
                return 0;
            }
        }
    }
...
#endif
{
    ...
    if (s->ext.ech.es != NULL) {
        if (s->version != TLS1_3_VERSION) {
            SSLfatal_data(s, SSL_AD_PROTOCOL_VERSION, ERR_R_UNSUPPORTED, "unsupported in client hello aux");
            return CON_FUNC_ERROR;
        }
        if ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0)
            sess_id_len = sizeof(s->tmp_session_id);
...
MSG_PROCESS_RETURN dtls_process_hello_verify(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (!PACKET_forward(pkt, 2)
        ...
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in client hello aux");
        return MSG_PROCESS_ERROR;
    }

    cookie_len = PACKET_remaining(&cookiepkt);
    if (cookie_len > sizeof(s->d1->cookie)) {
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_LENGTH, "invalid length in client hello aux");
        return MSG_PROCESS_ERROR;
    }
    ...
    const unsigned char *cipherchars)
{
    ...
    if (c == NULL) {
        /* unknown cipher */
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_UNKNOWN, "unknown in set client ciphersuite");
        return 0;
    }
    /*
    ...
    if (ssl_cipher_disabled(s, c, SSL_SECOP_CIPHER_CHECK, 1)) {
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_MISMATCH, "mismatch in set client ciphersuite");
        return 0;
    }
    ...
    if (s->hit && (s->session->cipher_id != c->id)) {
        if (SSL_CONNECTION_IS_TLS13(s)) {
            ...
            if (!ossl_assert(s->session->cipher != NULL)) {
                SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in set_client_ciphersuite");
                return 0;
            }
            /*
            ...
            if (md == NULL
                ...
                SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_CONFLICT, "error");
                return 0;
            }
        } else {
            /*
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (PACKET_peek_bytes(pkt, &shbuf, shlen) != 1) {
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in set client ciphersuite");
        goto err;
    }
#endif
    ...
    if (s->version == TLS1_3_VERSION
        ...
        if (s->hello_retry_request != SSL_HRR_NONE) {
            SSLfatal_data(s, SSL_AD_UNEXPECTED_MESSAGE, ERR_R_WRONG_STATE, "wrong state in set client ciphersuite");
            goto err;
        }
        s->hello_retry_request = SSL_HRR_PENDING;
    ...
    if (session_id_len > sizeof(s->session->session_id)
        ...
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_LENGTH, "invalid length in set client ciphersuite");
        goto err;
    }
    ...
    if (s->ext.ech.es != NULL
        ...
        if (!hrr && s->ext.ech.success == 1) {
             ...
             */
            SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_MISSING_REQUIRED_DATA, "missing required data in set client ciphersuite");
            goto err;
        } else {
    ...
    if (SSL_CONNECTION_IS_TLS13(s) || hrr) {
        if (compression != 0) {
            SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_ALGORITHM, "invalid algorithm in set client ciphersuite");
            goto err;
        }
        ...
        if (session_id_len != s->tmp_session_id_len
            || memcmp(PACKET_data(&session_id), s->tmp_session_id,
                   session_id_len)
                != 0) {
            SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_PASSED_INVALID_ARGUMENT, "invalid argument in set client ciphersuite");
            goto err;
        }
    }
    ...
    if (!tls_validate_all_contexts(s, context, extensions)) {
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_FORMAT, "invalid format in set client ciphersuite");
        goto err;
    }
    ...
    if (s->version != s->session->ssl_version) {
        SSLfatal_data(s, SSL_AD_PROTOCOL_VERSION, ERR_R_MISMATCH, "mismatch in set client ciphersuite");
        goto err;
    }
    /*
...
#ifdef OPENSSL_NO_COMP
    if (compression != 0) {
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_UNSUPPORTED, "unsupported in set client ciphersuite");
        goto err;
    }
    /*
    ...
    if (s->session->compress_meth != 0) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_CONFLICT, "conflict in set client ciphersuite");
        goto err;
    }
#else
    ...
    if (compression == 0)
        ...
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_DISABLED, "disabled in set client ciphersuite");
        goto err;
    } else {
    ...
    RAW_EXTENSION *extensions)
{
    ...
    if (s->ext.tls13_cookie_len == 0 && s->s3.tmp.pkey != NULL) {
         ...
         */
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_PROTOCOL_ERROR, "protocol error in set client ciphersuite");
        goto err;
    }
...
MSG_PROCESS_RETURN tls_process_server_rpk(SSL_CONNECTION *sc, PACKET *pkt)
{
    ...
    if (peer_rpk == NULL) {
        SSLfatal_data(sc, SSL_AD_DECODE_ERROR, ERR_R_INVALID_CERTIFICATE, "invalid certificate in set client ciphersuite");
        return MSG_PROCESS_ERROR;
    }
    ...
    WORK_STATE wst)
{
    ...
    if (sc->session->peer_rpk == NULL) {
        SSLfatal_data(sc, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_KEY, "invalid key in set client ciphersuite");
        return WORK_ERROR;
    }
    ...
    if (v_ok <= 0 && sc->verify_mode != SSL_VERIFY_NONE) {
        ERR_clear_last_mark();
        SSLfatal_data(sc, ssl_x509err2alert(sc->verify_result), ERR_R_CERTIFICATE_VERIFICATION_FAILED, "cert verification failed in set client ciphersuite");
        return WORK_ERROR;
    }
    ...
    if ((clu = ssl_cert_lookup_by_pkey(sc->session->peer_rpk, &certidx,
        ...
        == NULL) {
        SSLfatal_data(sc, SSL_AD_ILLEGAL_PARAMETER, ERR_R_UNKNOWN, "unknown in set client ciphersuite");
        return WORK_ERROR;
    }
    ...
    if (!SSL_CONNECTION_IS_TLS13(sc)) {
        if ((clu->amask & sc->s3.tmp.new_cipher->algorithm_auth) == 0) {
            SSLfatal_data(sc, SSL_AD_ILLEGAL_PARAMETER, ERR_R_WRONG_TYPE, "wrong type in set client ciphersuite");
            return WORK_ERROR;
        }
    }
    ...
    PACKET *pkt)
{
    ...
    if (s->ext.server_cert_type != TLSEXT_cert_type_x509) {
        SSLfatal_data(s, SSL_AD_UNSUPPORTED_CERTIFICATE, ERR_R_UNKNOWN, "unknown in set client ciphersuite");
        goto err;
    }

    if ((s->session->peer_chain = sk_X509_new_null()) == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in set client ciphersuite");
        goto err;
    }
    ...
    for (chainidx = 0; PACKET_remaining(pkt); chainidx++) {
        ...
        if (d2i_X509(&x, (const unsigned char **)&certbytes,
                cert_len)
            == NULL) {
            SSLfatal_data(s, SSL_AD_BAD_CERTIFICATE, ERR_R_ASN1_LIB, "error");
            goto err;
        }
    ...
    WORK_STATE wst)
{
    ...
    if (i <= 0 && s->verify_mode != SSL_VERIFY_NONE) {
        ERR_clear_last_mark();
        SSLfatal_data(s, ssl_x509err2alert(s->verify_result), ERR_R_CERTIFICATE_VERIFICATION_FAILED, "cert verification failed in set client ciphersuite");
        return WORK_ERROR;
    }
    ERR_pop_to_mark(); /* but we keep s->verify_result */
    ...
    if (pkey == NULL || EVP_PKEY_missing_parameters(pkey)) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_NOT_FOUND, "not found in set client ciphersuite");
        return WORK_ERROR;
    }
    ...
    if (!SSL_CONNECTION_IS_TLS13(s)) {
        if ((clu->amask & s->s3.tmp.new_cipher->algorithm_auth) == 0) {
            SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_WRONG_TYPE, "wrong type in set client ciphersuite");
            return WORK_ERROR;
        }
    }
...
static int tls_process_ske_psk_preamble(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (!PACKET_get_length_prefixed_2(pkt, &psk_identity_hint)) {
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in ske psk preamble");
        return 0;
    }
    ...
    if (PACKET_remaining(&psk_identity_hint) > PSK_MAX_IDENTITY_LEN) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_INVALID_LENGTH, "invalid length in ske psk preamble");
        return 0;
    }
    ...
    if (PACKET_remaining(&psk_identity_hint) == 0) {
        ...
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_process_ske_psk_preamble");
        return 0;
    }
...
static int tls_process_ske_srp(SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
    ...
    if (!PACKET_get_length_prefixed_2(pkt, &prime)
        ...
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in ske srp");
        return 0;
    }
    ...
    if ((s->srp_ctx.N = BN_bin2bn(PACKET_data(&prime),
        ...
        || (s->srp_ctx.B = BN_bin2bn(PACKET_data(&server_pub),
            ...
            == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_BN_LIB, "error");
        return 0;
    }
    ...
    return 1;
#else
    SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_process_ske_srp");
    return 0;
#endif
}
...
static int tls_process_ske_dhe(SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
    ...
    if (!PACKET_get_length_prefixed_2(pkt, &prime)
        ...
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in ske dhe");
        return 0;
    }
    ...
    if (tmpl == NULL
        ...
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_process_ske_dhe");
        goto err;
    }
    ...
    if (EVP_PKEY_fromdata_init(pctx) <= 0
        ...
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INVALID_KEY, "invalid key in ske dhe");
        goto err;
    }
    ...
    if (pctx == NULL
        ...
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_KEY, "invalid key in ske dhe");
        goto err;
    }
    ...
    if (!ssl_security(s, SSL_SECOP_TMP_DH,
            ...
            0, peer_tmp)) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_SECURITY_VIOLATION, "security violation in ske dhe");
        goto err;
    }
...
static int tls_process_ske_ecdhe(SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
    ...
    if (!PACKET_get_1(pkt, &curve_type) || !PACKET_get_net_2(pkt, &curve_id)) {
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in ske ecdhe");
        return 0;
    }
    /*
    ...
    if (curve_type != NAMED_CURVE_TYPE
        ...
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_MISMATCH, "mismatch in ske ecdhe");
        return 0;
    }

    if ((s->s3.peer_tmp = ssl_generate_param_group(s, curve_id)) == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_NOT_FOUND, "not found in ske ecdhe");
        return 0;
    }
    ...
    if (EVP_PKEY_set1_encoded_public_key(s->s3.peer_tmp,
        ...
        <= 0) {
        SSLfatal_data(s, SSL_AD_ILLEGAL_PARAMETER, ERR_R_INVALID_KEY, "invalid key in ske ecdhe");
        return 0;
    }
...
MSG_PROCESS_RETURN tls_process_key_exchange(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (alg_k & (SSL_kPSK | SSL_kRSAPSK)) {
        ...
        SSLfatal_data(s, SSL_AD_UNEXPECTED_MESSAGE, ERR_R_WRONG_STATE, "wrong state in ske ecdhe");
        goto err;
    }
    ...
    if (pkey != NULL) {
        ...
        if (!PACKET_get_sub_packet(&save_param_start, &params,
            ...
            SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_process_ske_ecdhe");
            goto err;
        }
        ...
        if (SSL_USE_SIGALGS(s)) {
            ...
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_UNSUPPORTED, "unsupported in ske ecdhe");
            goto err;
        }

        if (!tls1_lookup_md(sctx, s->s3.tmp.peer_sigalg, &md)) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_NO_MATCH, "no matching ske ecdhe");
            goto err;
        }
        if (SSL_USE_SIGALGS(s))
        ...
        if (md_ctx == NULL) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB, "EVP error in ske ecdhe");
            goto err;
        }
        ...
        OPENSSL_free(tbs);
        if (rv <= 0) {
            SSLfatal_data(s, SSL_AD_DECRYPT_ERROR, ERR_R_VERIFICATION_FAILED, "verification failed in ske ecdhe");
            goto err;
        }
        ...
        if (!(s->s3.tmp.new_cipher->algorithm_auth & (SSL_aNULL | SSL_aSRP))
            ...
            if (ssl3_check_cert_and_algorithm(s)) {
                SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_BAD_DATA, "bad data in ske ecdhe");
            }
        ...
        if (PACKET_remaining(pkt) != 0) {
            SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_PROTOCOL_ERROR, "protocol error in ske ecdhe");
            goto err;
        }
    }
    ...
    PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls1_process_sigalgs(s)) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INVALID_LENGTH, "invalid length in ske ecdhe");
            return MSG_PROCESS_ERROR;
        }
    } else {
        PACKET ctypes;
        ...
        if (!PACKET_memdup(&ctypes, &s->s3.tmp.ctype, &s->s3.tmp.ctype_len)) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_process_ske_ecdhe");
            return MSG_PROCESS_ERROR;
        }
        ...
        if (SSL_USE_SIGALGS(s)) {
            ...
            if (!tls1_save_sigalgs(s, &sigalgs, 0)) {
                SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INVALID_ALGORITHM, "invalid algorithm in ske ecdhe");
                return MSG_PROCESS_ERROR;
            }
            if (!tls1_process_sigalgs(s)) {
                SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_SSL_LIB, "SSL error in ske ecdhe");
                return MSG_PROCESS_ERROR;
            }
        }
    ...
    PACKET *pkt)
{
    ...
    if (s->session->ext.tick == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in ske ecdhe");
        goto err;
    }
...
int tls_process_cert_status_body(SSL_CONNECTION *s, size_t chainidx, PACKET *pkt)
{
    ...
    if (!PACKET_get_1(pkt, &type)
        ...
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_UNSUPPORTED, "unsupported in cert status body");
        return 0;
    }
    ...
    if (PACKET_remaining(pkt) > 0) {
        if (!PACKET_get_net_3_len(pkt, &resplen)
            ...
            SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in cert status body");
            return 0;
        }

        if (resplen == 0) {
            SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_BAD_DATA, "bad data in cert status body");
            return 0;
        }

        if ((respder = OPENSSL_malloc(resplen)) == NULL) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cert status body");
            return 0;
        }
        ...
        OPENSSL_free(respder);
        if (resp == NULL) {
            SSLfatal_data(s, TLS1_AD_BAD_CERTIFICATE_STATUS_RESPONSE, ERR_R_INVALID_FORMAT, "invalid format in cert status body");
            return 0;
        }
...
int tls_process_initial_server_flight(SSL_CONNECTION *s)
{
    ...
    if (s->ext.status_type != TLSEXT_STATUSTYPE_nothing
        ...
        if (ret == 0) {
            SSLfatal_data(s, SSL_AD_BAD_CERTIFICATE_STATUS_RESPONSE, ERR_R_INVALID_FORMAT, "invalid format in initial server flight");
            return 0;
        }
        if (ret < 0) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CALLBACK_FAILED, "callback failed in initial server flight");
            return 0;
        }
    }
    ...
    if (s->ext.ech.es != NULL
        ...
        SSLfatal_data(s, SSL_AD_ECH_REQUIRED, ERR_R_MISSING_REQUIRED_DATA, "missing required data in initial server flight");
        return 0;
    }
...
MSG_PROCESS_RETURN tls_process_server_done(SSL_CONNECTION *s, PACKET *pkt)
{
    if (PACKET_remaining(pkt) > 0) {
        ...
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in initial server flight");
        return MSG_PROCESS_ERROR;
    }
#ifndef OPENSSL_NO_SRP
    if (s->s3.tmp.new_cipher->algorithm_mkey & SSL_kSRP) {
        if (ssl_srp_calc_a_param_intern(s) <= 0) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_OPERATION_FAIL, "operation failed in initial server flight");
            return MSG_PROCESS_ERROR;
        }
    }
#endif
...
static int tls_construct_cke_psk_preamble(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (s->psk_client_callback == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_MISSING_REQUIRED_DATA, "missing required data in cke psk preamble");
        goto err;
    }
    ...
    if (psklen > PSK_MAX_PSK_LEN) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_psk_preamble");
        psklen = PSK_MAX_PSK_LEN; /* Avoid overrunning the array on cleanse */
        goto err;
    } else if (psklen == 0) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_NOT_FOUND, "not found in cke psk preamble");
        goto err;
    }

    identitylen = strlen(identity);
    if (identitylen > PSK_MAX_IDENTITY_LEN) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_psk_preamble");
        goto err;
    }
    ...
    if (tmppsk == NULL || tmpidentity == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cke psk preamble");
        goto err;
    }
...
static int tls_construct_cke_rsa(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (!received_server_cert(s)) {
         ...
         */
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_rsa");
        return 0;
    }
    ...
    if (pms == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cke rsa");
        return 0;
    }
    ...
    if (RAND_bytes_ex(sctx->libctx, pms + 2, pmslen - 2, 0) <= 0) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_RAND_LIB, "error");
        goto err;
    }
    ...
    if (pctx == NULL || EVP_PKEY_encrypt_init(pctx) <= 0
        ...
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB, "EVP error in cke rsa");
        goto err;
    }
    if (!WPACKET_allocate_bytes(pkt, enclen, &encdata)
        || EVP_PKEY_encrypt(pctx, encdata, &enclen, pms, pmslen) <= 0) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_OPERATION_FAIL, "operation failed in cke rsa");
        goto err;
    }
    EVP_PKEY_CTX_free(pctx);
    pctx = NULL;
...
static int tls_construct_cke_dhe(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    skey = s->s3.peer_tmp;
    if (skey == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_dhe");
        goto err;
    }
...
static int tls_construct_cke_ecdhe(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    skey = s->s3.peer_tmp;
    if (skey == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_ecdhe");
        return 0;
    }

    ckey = ssl_generate_pkey(s, skey);
    if (ckey == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_SSL_LIB, "SSL error in cke ecdhe");
        goto err;
    }
    ...
    if (encoded_pt_len == 0) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_EC_LIB, "EC error in cke ecdhe");
        goto err;
    }
...
static int tls_construct_cke_gost(SSL_CONNECTION *s, WPACKET *pkt)
{
     ...
     */
    if ((pkey = tls_get_peer_pkey(s)) == NULL) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_MISSING_REQUIRED_DATA, "missing required data in cke gost");
        return 0;
    }
    ...
    pkey_ctx = EVP_PKEY_CTX_new_from_pkey(sctx->libctx,
        ...
        sctx->propq);
    if (pkey_ctx == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB, "EVP error in cke gost");
        return 0;
    }
    /*
    ...
    pms = OPENSSL_malloc(pmslen);
    if (pms == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cke gost");
        goto err;
    }
    ...
    if (EVP_PKEY_encrypt_init(pkey_ctx) <= 0
        ...
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_gost");
        goto err;
    };
    /*
...
static int tls_construct_cke_gost18(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (cipher_nid == NID_undef) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_gost18");
        return 0;
    }
    ...
    pms = OPENSSL_malloc(pmslen);
    if (pms == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cke gost18");
        goto err;
    }
    ...
    /* Get server certificate PKEY and create ctx from it */
    if ((pkey = tls_get_peer_pkey(s)) == NULL) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_MISSING_REQUIRED_DATA, "missing required data in cke gost18");
        goto err;
    }
    ...
    pkey_ctx = EVP_PKEY_CTX_new_from_pkey(sctx->libctx,
        ...
        sctx->propq);
    if (pkey_ctx == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB, "EVP error in cke gost18");
        goto err;
    }
...
static int tls_construct_cke_srp(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (s->srp_ctx.A == NULL
        || !WPACKET_sub_allocate_bytes_u16(pkt, BN_num_bytes(s->srp_ctx.A),
            &abytes)) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_construct_cke_srp");
        return 0;
    }
    ...
    if (s->session->srp_username == NULL) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CRYPTO_LIB, "crypto error in cke srp");
        return 0;
    }
...
int tls_client_key_exchange_post_work(SSL_CONNECTION *s)
{
    ...
    if (pms == NULL && !(s->s3.tmp.new_cipher->algorithm_mkey & SSL_kPSK)) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_PASSED_INVALID_ARGUMENT, "invalid argument in client key exchange post work");
        goto err;
    }
    ...
    if (SSL_CONNECTION_IS_DTLS(s)) {
        ...
        if (SSL_export_keying_material(ssl, sctpauthkey,
                ...
                labellen, NULL, 0, 0)
            <= 0) {
            SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in tls_client_key_exchange_post_work");
            goto err;
        }
...
WORK_STATE tls_prepare_client_certificate(SSL_CONNECTION *s, WORK_STATE wst)
{
    ...
    if (wst == WORK_MORE_A) {
        ...
        if (s->cert->cert_cb) {
            ...
            if (i == 0) {
                SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_CALLBACK_FAILED, "callback failed in check client certificate");
                return WORK_ERROR;
            }
            s->rwstate = SSL_NOTHING;
        }
    ...
    SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in ssl3_check_client_certificate");
    return WORK_ERROR;
}
    ...
    WPACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)
         ...
         */
        SSLfatal_data(s, SSL_AD_NO_ALERT, ERR_R_UNSUPPORTED, "unsupported in check client certificate");
        return CON_FUNC_ERROR;
    }
    ...
    WPACKET *pkt)
{
    ...
    if (SSL_IS_FIRST_HANDSHAKE(sc)
         ...
         */
        SSLfatal_data(sc, SSL_AD_NO_ALERT, ERR_R_UNSUPPORTED, "unsupported in check client certificate");
        goto out;
    }
    ret = 1;
    goto out;
...
err:
    SSLfatal_data(sc, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in ssl3_check_client_certificate");
out:
    if (buf != NULL) {
...
int ssl3_check_cert_and_algorithm(SSL_CONNECTION *s)
{
    ...
    if (clu == NULL || (alg_a & clu->amask) == 0) {
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_MISSING_REQUIRED_DATA, "missing required data in check cert and algorithm");
        return 0;
    }
    ...
    if ((alg_k & SSL_kDHE) && (s->s3.peer_tmp == NULL)) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR, "internal error in ssl3_check_cert_and_algorithm");
        return 0;
    }
    ...
    if (clu->amask & SSL_aECDSA) {
        if (ssl_check_srvr_ecc_cert_and_alg(s->session->peer, s))
            return 1;
        SSLfatal_data(s, SSL_AD_HANDSHAKE_FAILURE, ERR_R_INVALID_CERTIFICATE, "invalid certificate in check cert and algorithm");
        return 0;
    }
...
MSG_PROCESS_RETURN tls_process_hello_req(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (PACKET_remaining(pkt) > 0) {
        /* should contain no data */
        SSLfatal_data(s, SSL_AD_DECODE_ERROR, ERR_R_INVALID_LENGTH, "invalid length in check cert and algorithm");
        return MSG_PROCESS_ERROR;
    }
...
CON_FUNC_RETURN tls_construct_end_of_early_data(SSL_CONNECTION *s, WPACKET *pkt)
{
    if (s->early_data_state != SSL_EARLY_DATA_WRITE_RETRY
        && s->early_data_state != SSL_EARLY_DATA_FINISHED_WRITING) {
        SSLfatal_data(s, SSL_AD_INTERNAL_ERROR, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED, "error");
        return CON_FUNC_ERROR;
    }
